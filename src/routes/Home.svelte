        ...
        {#if foundIds.includes(entry.id) || searchString === ""}
            <div class="entry">
                <div class="title-container">
                    <h2>{entry.title}</h2>
                    <div>{DateTime.fromMillis(entry.date).toLocaleString(DateTime.DATETIME_FULL)}</div>
                    <div on:click={() => handleShowHide(i)}>&nbsp;{openContent === i ? '▲' : '▼'}</div>
                </div>

                <div class:is-open={openContent === i} class="content">{@html parse(entry.content)}</div>
            </div>
        {/if}
    {/each}
{/if}
